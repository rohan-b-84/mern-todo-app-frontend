...
const NoteState = (props) => {
  ...
  const deleteNote = async (id) => {
    ...
    try {
      let token = localStorage.getItem("token");
      let url = `http://localhost:5000/api/task/delete-task/${newID}`;
  ...
  const editNote = async (note) => {
    try {
      ...
      let response = await fetch(
        `http://localhost:5000/api/task/update-task/${note._id}`,
        {
          method: "PUT",
          headers: {
            "auth-token": token,
  ...
  const deleteAll = async () => {
    try {
      ...
      await fetch(`http://localhost:5000/api/task/delete-all/`, {
        method: "DELETE",
        headers: {
          "auth-token": token,
          "Content-Type": "application/json",
  ...
  const deleteCompleted = async () => {
    try {
      const token = localStorage.getItem("token");
      let response = await fetch(
        `http://localhost:5000/api/task/delete-completed/`,
        {
          method: "DELETE",
          headers: {
            "auth-token": token,
